      ...
      <table class="table table-hover table-bordered table-condensed">
        ...
        <tbody>
          <tr
            ...
            :class="{ success: selectedIndex == index }"
            :key="index"
          >
            <td>
              ...
              <button class="btn btn-primary" @click="displayProgress(index)">
                <i class="fa fa-truck" aria-hidden="true"></i>&nbsp;進度
              </button>
              ...
              <button
                ...
                @click="closeOrder(index)"
                v-if="order.active"
              >
                <i class="fa fa-money"></i>&nbsp;結案
              </button>
              <button
              ...
              <button
                class="btn btn-danger"
                @click="reopenOrder(index)"
                v-if="!order.active"
              >
                <i class="fa fa-repeat"></i>&nbsp;重啟
              </button>
            </td>
            <td>{{ order._id }}</td>
            ...
            <td>
              <div class="progress">
                <div
                  ...
                  role="progressbar"
                  :aria-valuenow="productionPercent(order)"
                  aria-valuemin="0"
                  aria-valuemax="100"
...
export default {
  ...
  methods: {
    ...
    closeOrder(idx) {
      axios
        .post('/CloseOrder', this.orderList[idx])
        .then(resp => {
          const ret = resp.data;
          if (ret.ok) {
            alert('訂單結案');
    ...
    reopenOrder(idx) {
      axios
        .post('/ReopenOrder/' + this.orderList[idx]._id)
        .then(resp => {
          const ret = resp.data;
          if (ret.ok) {
            alert('訂單重啟');
    ...
    deleteOrder(idx) {
      axios
        .delete('/Order/' + this.orderList[idx]._id)
        .then(resp => {
          const ret = resp.data;
          if (ret.ok) {
            alert('訂單刪除');
